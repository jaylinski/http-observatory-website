...
const insertHTTP = async (stats) => {
  ...
  const createTable = (title, alert) => {
    ...
    var table = $('<table></table>')
      .addClass('table table-bordered table-striped table-recent-results')
      .append(
        '<thead><tr><th class="alert-' + alert + ' h5" colspan="2">' + title + '</th></tr></thead>'
      );
    ...
    [
      {
        ...
        alert: 'primary',
        data: stats.recent.scans.recent,
        id: 'results-recent',
      },
      {
        name: 'High Achievers',
        alert: 'success',
        data: stats.recent.scans.best,
        id: 'results-good',
      },
      {
        name: 'Failing Grade',
        alert: 'danger',
        data: stats.recent.scans.worst,
        id: 'results-bad',
      },
    ],
    (t) => {
  ...
  stats.misc.percSitesPassing =
    (1 - stats.gradeDistribution.latest.F / stats.misc.numUniqueSites) * 100;
  ...
  new Chart($('#http-observatory-chart-grade-distribution'), {
    ...
    data: {
      ...
      datasets: [
        {
          label: ' ',
          data: nonFailingGrades.map((k) => {
            return stats.gradeDistribution.latest[k];
          }),
          backgroundColor: [
            colors.A,
            colors.A,
      ...
      datasets: [
        {
          label: ' ',
          data: [
            stats.gradeImprovements['1'],
            stats.gradeImprovements['2'],
            stats.gradeImprovements['3'],
            stats.gradeImprovements['4'],
            stats.gradeImprovements['5'],
          ],
